<template>
    <div class="restoration-preview">
      ...
      <div class="restoration-preview__content">
        <div v-if="isProcessing" class="restoration-preview__processing">
          ...
          <p class="restoration-preview__progress-text">
            正在修复中 ({{ Math.round(progress) }}%)
          </p>
        </div>
        ...
        <div v-else-if="originalImage && restoredImage" class="restoration-preview__comparison">
          <div class="restoration-preview__image-container">
            <div class="restoration-preview__image-wrapper">
              <img src="https://typora-hhhhry.oss-cn-beijing.aliyuncs.com/OIP-C%20(7).jpg" alt="" class="restoration-preview__image" />
              <div class="restoration-preview__image-label">原始壁画</div>
            </div>
            <div class="restoration-preview__image-wrapper">
              <img src="https://typora-hhhhry.oss-cn-beijing.aliyuncs.com/OIP-C%20(3).jpg" alt="" class="restoration-preview__image" />
              <div class="restoration-preview__image-label restoration-preview__image-label--restored">修复后壁画</div>
            </div>
          </div>

          <div class="restoration-preview__slider-container">
            <div class="restoration-preview__slider-wrapper">
              <div class="restoration-preview__slider-image" :style="{ backgroundImage: `url(${originalImage})` }"></div>
              <div class="restoration-preview__slider-image restoration-preview__slider-image--after" :style="{ backgroundImage: `url(${restoredImage})`, width: `${sliderPosition}%` }"></div>
              <div class="restoration-preview__slider-handle" :style="{ left: `${sliderPosition}%` }">
                <div class="restoration-preview__slider-handle-line"></div>
              </div>
              <input
                type="range"
                min="0"
                ...
                class="restoration-preview__slider-input"
              />
            </div>
            <div class="restoration-preview__slider-labels">
              <span>原始壁画</span>
              <span>修复后壁画</span>
            </div>
          </div>
  ...
  // 当修复完成时，重置滑块位置
  watch(() => props.restoredImage, (newValue) => {
    if (newValue) {
